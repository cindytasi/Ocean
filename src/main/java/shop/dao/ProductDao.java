...
//				product.setComId(rs.getInt("comId"));			
	...
	public List<Product> getAll() {
		...
		try (Connection conn = dataSource.getConnection(); PreparedStatement ps = conn.prepareStatement(getAll);) {
			ResultSet rs = ps.executeQuery();
	...
	public List<Product> allGender(int gender) {
		...
		try (Connection conn = dataSource.getConnection(); PreparedStatement ps = conn.prepareStatement(allGender);) {
			ps.setInt(1, gender);
			ResultSet rs = ps.executeQuery();
	...
	public List<Product> typeOfAcc(int specType) {
		...
		try (Connection conn = dataSource.getConnection(); PreparedStatement ps = conn.prepareStatement(typeOfAcc);) {
	...
	// 關鍵字查詢
	public List<Product> getKeyWord(String msg) {
		List<Product> result = new ArrayList<Product>();
		String keyWord = """
				SELECT productName, colorType, productImgId, price
				FROM (
					 	SELECT * FROM ProductInformation WHERE productName LIKE ? OR videoName LIKE ?
				) specTypeInfo
				GROUP BY productName, colorType, productImgId, price
				ORDER BY MAX(addedTime) DESC;
				""";
		try (Connection conn = dataSource.getConnection(); PreparedStatement ps = conn.prepareStatement(keyWord);) {
			ps.setString(1, "%" + msg + "%");
			ps.setString(2, "%" + msg + "%");
			ResultSet rs = ps.executeQuery();
